...
| [devgeta-on-path-in-non-interactive-shells.md](devgeta-on-path-in-non-interactive-shells.md) | zsh only: an AI coding agent, hook, cron job or launchd task reports `devgeta` as not on PATH while it is installed and working. Check with the one-line no-profile test at the top of that guide.                                              |
