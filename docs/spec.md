...
starts, one short line per tool call it makes (the tool and what it was pointed at, e.g.
`[1/2]   read internal/x.go`), and a closing line with the outcome, elapsed time, tool
count, and what the run cost as OpenCode reported it. The lines are rendered from the
`--format json` event stream as it arrives (`CommandParams.OnStdoutLine`), not after the
run, so a multi-minute round reads as working rather than wedged. The reviewer's own prose
is never printed there — findings belong in the journal — and stdout is unaffected, staying
