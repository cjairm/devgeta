...
5. **Every call that touches the PR names it — the reads as much as the post.** `--pr <n>` is mandatory on every `devgeta task` command here that resolves a PR: `pr-view` in step 2, `review-threads` in step 3, `submit-review` in step 6, and a `devgeta task comment-pr` if you post one. Without a number those commands resolve the PR from the checked-out branch, and with `--target` that branch is not this PR: an omitted number does not error. On the post it lands a correctly-written review on whatever PR the checkout happens to have open; on a read it hands you another PR's description and threads, so step 3 dedups this review against feedback from a PR nobody involved is looking at — findings already handled here get raised again, and an unrelated thread can look like a duplicate and drop a live one. So a number must be in hand here — if none was passed with `--target`, stop and ask for one rather than letting step 1 infer it from the branch.
...
With `--target`, `--pr PR_NUMBER` is not optional on that read — the checkout is not this PR, so an omitted number describes whatever PR the checkout branch has open, and you would be reading another PR's purpose, description, and review state.
...
```bash
devgeta task review-threads --state all      # add --pr PR_NUMBER if you have one
```

With `--target`, `--pr PR_NUMBER` is not optional on that read — the checkout is not this PR, so an omitted number returns another PR's threads and the whole dedup below runs against the wrong feedback: a finding this PR genuinely already addressed goes unrecognized and gets raised a second time, and an unrelated thread can read as a duplicate and drop a live finding.
...
- **A prior concern is still live in the code and is a real blocker → don't approve** (case 1 above). Flag the concern itself in one brief review — `devgeta task submit-review --event comment`, with a body saying the rest looks good, naming the one outstanding item, and committing to approve once it's addressed — rather than re-listing each thread or asking for resolutions. It must be that review submission and **not** `devgeta task comment-pr`: a top-level comment is not a review, so it leaves the review request pending, and an unattended caller that keys off the request — `/pr-review-loop` — sees it still open and posts the same note again on the next tick. With `--target`, `--pr PR_NUMBER` is not optional on that call either — the checkout is not this PR, so an omitted number posts the review to whatever PR the checkout branch has open.
