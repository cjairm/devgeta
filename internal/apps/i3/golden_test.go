...
// repoDefaultI3Palette isolates paths.Paths.App.Configs.Themes/.Neovim and
// paths.Paths.Config.Nvim at the real repo's configs/, loads the real
// shipped "default" theme, and returns the repo root plus i3's resolved
...
func repoDefaultI3Palette(t *testing.T) (repoRoot string, p theme.Palette) {
	t.Helper()
	...
	p, err = def.PaletteFor("i3")
	if err != nil {
		t.Fatalf("failed to resolve i3's palette: %v", err)
	}
	return repoRoot, p
}

// TestForceConfigureTheme_MatchesGoldenRender renders the real shipped
// config.tmpl with the real "default" theme's real palette and compares it
// byte-for-byte against a committed golden. A change to either the
...
// testdata/golden_i3_config deliberately when the change is intended.
func TestForceConfigureTheme_MatchesGoldenRender(t *testing.T) {
	repoRoot, p := repoDefaultI3Palette(t)

	out := filepath.Join(t.TempDir(), "config")
	tmplPath := filepath.Join(repoRoot, "configs", "i3", "config.tmpl")
	if err := files.GenerateFromTemplate(tmplPath, out, map[string]any{"Palette": p}); err != nil {
		t.Fatalf("failed to render i3 config.tmpl: %v", err)
	}
	...
	goldenPath := filepath.Join("testdata", "golden_i3_config")
	...
	if string(got) != string(want) {
		t.Errorf(
			"rendered i3 config differs from %s\n--- got ---\n%s\n--- want ---\n%s",
			goldenPath, got, want,
		)
	}
}
