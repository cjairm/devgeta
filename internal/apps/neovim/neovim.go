...
var (
	_ apps.App              = (*Neovim)(nil)
	_ apps.ThemedConfigurer = (*Neovim)(nil)
)
...
func (n *Neovim) ForceConfigure() error {
	...
	return n.ForceConfigureTheme(def)
}

func (n *Neovim) ForceConfigureTheme(def theme.Definition) error {
	...
	if err := files.CopyDir(paths.Paths.App.Configs.Neovim, paths.Paths.Config.Nvim); err != nil {
		return err
	}
	// The shim init.lua requires unconditionally (configs/neovim/init.lua:
	// require("devgeta.theme")). Written after CopyDir so it always names the
	// current theme's module regardless of what CopyDir shipped or left in
	// place from a prior theme.
	shimContent := fmt.Sprintf(
		"-- [devgeta]\n"+
			"-- Generated by `dg theme set` / `dg configure neovim --force`.\n"+
			"-- Do not edit - set neovim_module in the theme's yaml file instead.\n"+
			"require(\"devgeta.themes.%s\")\n"+
			"-- [/devgeta]\n",
		def.NeovimModule,
	)
	shimPath := filepath.Join(paths.Paths.Config.Nvim, "lua", "devgeta", "theme.lua")
	if err := files.WriteFileAtomic(shimPath, []byte(shimContent), 0o644); err != nil {
		return fmt.Errorf("failed to write neovim theme shim: %w", err)
	}
	return nil
}
